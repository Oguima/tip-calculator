...
class SplitInputView: UIView {
    ...
    private lazy var decrementButton: UIButton = {
        let button = buildButton(text: "-", corners: [
            .layerMinXMaxYCorner,
            .layerMinXMinYCorner,
        ])
    ...
    private let splitSubject: CurrentValueSubject<Int, Never> = .init(1) // Valor default
    ...
    private func observe() {
        splitSubject.sink { [unowned self] quantity in
            quantityLabel.text = quantity.stringValue // Int extension
        }.store(in: &cancellables)
    }
